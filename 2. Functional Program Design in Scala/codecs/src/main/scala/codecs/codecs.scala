...
trait EncoderInstances {
  ...
  implicit val unitEncoder: Encoder[Unit] = Encoder.fromFunction(_ => Json.Null)

  /** An encoder for `Int` values */
  implicit val intEncoder: Encoder[Int] = Encoder.fromFunction(n => Json.Num(BigDecimal(n)))

  /** An encoder for `String` values */
  // TODO Implement the `Encoder[String]` instance
  implicit val stringEncoder: Encoder[String] = Encoder.fromFunction(str => Json.Str(str))

  /** An encoder for `Boolean` values */
  // TODO Define an implicit value of type `Encoder[Boolean]`
  implicit val booleanEncoder: Encoder[Boolean] = Encoder.fromFunction(boolean => Json.Bool(boolean))
...
trait DecoderInstances {
  ...
  implicit val unitDecoder: Decoder[Unit] = Decoder.fromPartialFunction { case Json.Null => () }

  /** A decoder for `Int` values. Hint: use the `isValidInt` method of `BigDecimal`. */
  // TODO Define an implicit value of type `Decoder[Int]`
  implicit val intDecoder: Decoder[Int] = Decoder.fromPartialFunction {case Json.Num(n) if n.isValidInt => n.intValue}
//  implicit val intDecoder: Decoder[Int] = Decoder.fromFunction(v => {
//    case Json.Num(v) => if (v.isValidInt) Some(v.intValue) else None
//    case _ => None
//  })

  /** A decoder for `String` values */
  // TODO Define an implicit value of type `Decoder[String]`
  implicit val stringDecoder: Decoder[String] = Decoder.fromPartialFunction {case Json.Str(v) => v}

  /** A decoder for `Boolean` values */
  // TODO Define an implicit value of type `Decoder[Boolean]`
  implicit val booleanDecoder: Decoder[Boolean] = Decoder.fromPartialFunction {case Json.Bool(v) => v.booleanValue()}
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(arr) => Some(arr.map(x => decoder.decode(x).get))
      case _ => None
    }
