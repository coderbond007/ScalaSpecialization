...
object WikipediaRanking extends WikipediaRankingInterface {
    ...
    val wikiRdd: RDD[WikipediaArticle] = sc.parallelize(WikipediaData.lines).map(cur => WikipediaData.parse(cur))
    ...
    def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd
            .filter(cur => cur.mentionsLanguage(lang))
            .aggregate(0)((x, _) => x + 1, _ + _)
    ...
    def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
        rdd
                .flatMap(
                    x => langs
                            .filter(cur => x.mentionsLanguage(cur))
                            .map((_, x))
                )
                .groupByKey()
