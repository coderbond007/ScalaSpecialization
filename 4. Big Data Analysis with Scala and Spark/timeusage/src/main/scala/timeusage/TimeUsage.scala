...
import org.apache.spark.sql.functions.{avg, column, round, when}
...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primary: List[String] = List("t01", "t03", "t11", "t1801", "t1803")
    val working: List[String] = List("t05", "t1805")
    val other: List[String] = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    val grouped = columnNames
            .groupBy(cur => {
              if (primary.exists(cur.startsWith)) "primary"
              else if (working.exists(cur.startsWith)) "working"
              else if (other.exists(cur.startsWith)) "other"
              else "no"
            })
            .mapValues(_.map(column))
    (grouped("primary"), grouped("working"), grouped("other"))
  }
